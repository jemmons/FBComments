...
- (id)parseJsonResponse:(NSData *)data error:(NSError **)error {
  ...
  //Reasonable sanity check. Even assuming 4 bytes/character for "true" or "false"
  if([data length] < 24){
  ...
  NSError *anError = nil;
  id result = [NSJSONSerialization JSONObjectWithData:data options:0 error:&anError];
  if(nil == result){
    NSLog(@"Failing silently to parse Facebook JSON:\n%@", anError);
  }
